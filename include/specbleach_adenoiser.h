...
typedef struct SpectralBleachParameters {
  ...
  /* Sets the amount of dBs that the rejected audio will be attenuated. It goes
   * from 0 dB to 30 dB. */
  ...
  /* Unitless total oversubtraction strength from 0 to 2. */
  float noise_rescale;
  ...
  /* Runtime gate for the post-filter stage (blurs musical noise on low SNR
   * frames). The stage is allocated regardless of this flag; it only controls
   * whether the stage is applied. The POSTFILTER_ENABLED compile-time gate in
   * configurations.h must also be 1 for the stage to run at all. */
  bool post_filter_enabled;

  /* Enables the tone-kill continuous-tone suppressor. When enabled, a stable
   * dominant tone in the 600-800 Hz band is nulled in the input spectrum
   * before estimation, so it never reaches the gain or the mixer. */
