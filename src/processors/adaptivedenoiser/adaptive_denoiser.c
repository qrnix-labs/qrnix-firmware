...
spectral_adaptive_denoiser_initialize(const uint32_t sample_rate,
  ...
  self->tone_kill =
      tone_kill_initialize(self->sample_rate, self->fft_size, self->hop);
...
bool get_adaptive_noise_scaling_diagnostics(
    SpectralProcessorHandle instance, NoiseScalingDiagnostics *diagnostics) {
  if (!instance) {
    return false;
  }
  SpectralAdaptiveDenoiser *self = (SpectralAdaptiveDenoiser *)instance;
  if (!get_noise_scaling_diagnostics(self->noise_scaling_criteria,
                                     diagnostics)) {
    return false;
  }

  float gain_sum = 0.F;
  float mixed_gain_sum = 0.F;
  const uint32_t bin_count = self->real_spectrum_size - 1U;
  for (uint32_t k = 1U; k < self->real_spectrum_size; k++) {
    const float gain = self->gain_spectrum[k];
    gain_sum += gain;
    mixed_gain_sum += gain + (1.F - gain) * self->parameters.reduction_amount;
  }
  if (bin_count > 0U) {
    diagnostics->average_gain = gain_sum / (float)bin_count;
    diagnostics->average_mixed_gain = mixed_gain_sum / (float)bin_count;
  }
  return true;
}
...
bool spectral_adaptive_denoiser_run(SpectralProcessorHandle instance,
  ...
  // Tone-kill: null a continuous dominant tone in the input spectrum before
  // it reaches the reference spectrum, the noise estimator, the gain, or the
  // mixer.
  tone_kill_run(self->tone_kill, fft_spectrum,
                self->parameters.tone_kill_enabled);
  ...
  NoiseScalingParameters oversubtraction_parameters = (NoiseScalingParameters){
      .oversubtraction = self->parameters.noise_rescale,
  ...
  // Apply optional post filtering to reduce residual noise on low SNR frames.
#if POSTFILTER_ENABLED
  if (self->parameters.post_filter_enabled) {
