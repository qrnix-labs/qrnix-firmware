...
bool specbleach_adaptive_get_diagnostics(
    SpectralBleachHandle instance, SpectralBleachDiagnostics *diagnostics) {
  if (!instance || !diagnostics) {
    return false;
  }
  SbAdaptiveDenoiser *self = (SbAdaptiveDenoiser *)instance;
  NoiseScalingDiagnostics internal;
  if (!get_adaptive_noise_scaling_diagnostics(
          self->adaptive_spectral_denoiser, &internal)) {
    return false;
  }
  diagnostics->minimum_snr = internal.minimum_snr;
  diagnostics->average_snr = internal.average_snr;
  diagnostics->maximum_snr = internal.maximum_snr;
  diagnostics->aggression_bands = internal.aggression_bands;
  diagnostics->bypassed_bands = internal.bypassed_bands;
  diagnostics->average_gain = internal.average_gain;
  diagnostics->average_mixed_gain = internal.average_mixed_gain;
  return true;
}
...
bool specbleach_adaptive_load_parameters(SpectralBleachHandle instance,
  ...
  self->denoise_parameters = (AdaptiveDenoiserParameters){
      ...
      .reduction_amount =
          from_db_to_amplitude_coefficient(parameters.reduction_amount * -1.F),
      ...
      .post_filter_enabled = parameters.post_filter_enabled,
      .tone_kill_enabled = parameters.tone_kill_enabled,
  };
  // clang-format on
