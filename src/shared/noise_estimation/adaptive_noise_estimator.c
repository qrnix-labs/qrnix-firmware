...
bool louizou_estimator_run(AdaptiveNoiseEstimator *self, const float *spectrum,
  ...
  // Seed the minimum tracker and noise estimate from the first real frame.
  // Starting the local minimum at FLT_MIN makes every first-frame bin appear
  // to contain speech, after which the estimator protects nearly all input
  // and never establishes a useful noise floor.
  if (!self->initialized) {
    memcpy(self->current->smoothed_spectrum, spectrum,
           sizeof(float) * self->noise_spectrum_size);
    memcpy(self->current->local_minimum_spectrum, spectrum,
           sizeof(float) * self->noise_spectrum_size);
    memset(self->current->speech_present_probability_spectrum, 0,
           sizeof(float) * self->noise_spectrum_size);
    memcpy(noise_spectrum, spectrum,
           sizeof(float) * self->noise_spectrum_size);
