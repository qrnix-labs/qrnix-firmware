...
struct NoiseScalingCriterias {
  ...
  NoiseScalingDiagnostics diagnostics;
...
static void a_posteriori_snr_critical_bands(NoiseScalingCriterias *self,
  ...
  float snr_sum = 0.F;
  uint32_t valid_bands = 0U;

  self->diagnostics = (NoiseScalingDiagnostics){
      .minimum_snr = FLT_MAX,
      .maximum_snr = -FLT_MAX,
  };
  ...
  for (uint32_t j = 0U; j < self->number_critical_bands; j++) {
    ...
    if (isfinite(a_posteriori_snr)) {
      self->diagnostics.minimum_snr =
          fminf(self->diagnostics.minimum_snr, a_posteriori_snr);
      self->diagnostics.maximum_snr =
          fmaxf(self->diagnostics.maximum_snr, a_posteriori_snr);
      snr_sum += a_posteriori_snr;
      valid_bands++;
    }
    ...
    oversustraction_factor = fmaxf(oversustraction_factor, 0.F);

    if (a_posteriori_snr > self->higher_snr) {
      self->diagnostics.bypassed_bands++;
    } else {
      self->diagnostics.aggression_bands++;
    }
  ...
  if (valid_bands > 0U) {
    self->diagnostics.average_snr = snr_sum / (float)valid_bands;
  } else {
    self->diagnostics.minimum_snr = 0.F;
    self->diagnostics.maximum_snr = 0.F;
  }
}

bool get_noise_scaling_diagnostics(const NoiseScalingCriterias *self,
                                   NoiseScalingDiagnostics *diagnostics) {
  if (!self || !diagnostics) {
    return false;
  }
